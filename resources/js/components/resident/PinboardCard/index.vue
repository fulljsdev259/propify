<template>
    <el-card  :class="{announcement: data.announcement}">
        <div ref="container">
            ...
            <read-more class="content" :text="data.content" :max-chars="512" :more-str="$t('resident.read_more')" :less-str="$t('resident.read_less')"/>

            <hr v-if="data.announcement"/>
            <div class="execution" v-if="data.announcement">
                {{$t('resident.execution')}} {{execution}}
            </div>
            <div class="providers" v-if="data.announcement && data.providers && data.providers.length">
                {{$t('resident.providers')}}: {{data.providers.map(provider => provider.name).join(', ')}}
            </div>
...
<script>
    ...
    import Likes from 'components/resident/Likes'
